    ...
    public function addFriend()
    {
        ...
        if (!$friend) {
            throw ValidationException::withMessages([
                'form.add-friend' => __('validations.user_not_found'),
            ]);
        }
        ...
        if ($friendship) {
            if ($friendship->status == 'pending') {
                throw ValidationException::withMessages([
                    'form.add-friend' => __('validations.friend_request_pending'),
                ]);
            } elseif ($friendship->status == 'accepted') {
                throw ValidationException::withMessages([
                    'form.add-friend' => __('validations.already_friends'),
                ]);
            }
            elseif ($friendship->status == 'blocked') {
                throw ValidationException::withMessages([
                    'form.add-friend' => __('validations.friend_request_blocked'),
                ]);
            }
        }
